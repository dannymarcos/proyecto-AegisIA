from execute_trade import request
...
def monitor_market():
    ...
    logger.info("Starting real-time market monitoring thread")
    ...
    while market_monitor_active:
        ...
        try:
            # Get current trading mode from Flask app config
            trading_mode = current_app.config.get("TRADING_MODE", "spot")
            logger.info(f"Current trading mode: {trading_mode}")
            
            if trading_mode == 'futures':
                # Check if futures API keys are configured
                ...
                if not futures_api_key or not futures_private_key:
                    logger.error("Futures API keys not configured")
                    time.sleep(5)
                    continue
                ...
                if not tickers or 'tickers' not in tickers:
                    logger.error("Could not get futures market data")
                    time.sleep(5)
                    continue
                ...
                if historical_data.empty:
                    logger.error("Could not get futures historical data")
                    time.sleep(5)
                    continue
                # Get account information for futures
                accounts = futures_client.get_accounts()
                if not accounts or 'accounts' not in accounts:
                    logger.error("Could not get futures account information")
                    time.sleep(5)
                    continue
                    
                available_margin = float(accounts['accounts'][0].get('auxiliary', {}).get('usd', 0))
                logger.info(f"Available margin for futures: {available_margin} USD")
            else:
                # Use spot trading API
                historical_data = KrakenAPI.fetch_historical_data("XBTUSD", 60, int(time.time() - 3600))
            ...
            if historical_data is not None and not historical_data.empty:
                ...
                if last_market_conditions:
                    ...
                    if (price_change_since_last > strategy_change_threshold or
                        ...
                        (current_conditions['macd'] * last_market_conditions['macd'] < 0)):  # MACD crossover
                        
                        logger.info("Significant market change detected. Re-evaluating strategy...")
                        logger.info(f"Price change: {price_change_since_last*100:.2f}%")
                        logger.info(f"New patterns: {current_conditions['patterns']}")
                        logger.info(f"RSI change: {current_conditions['rsi'] - last_market_conditions['rsi']:.2f}")
                        logger.info(f"MACD crossover: {current_conditions['macd']:.2f}")
                        
                        # Reset strategy to force re-evaluation
                        current_strategy = None
                        last_action = None
                
                last_market_conditions = current_conditions
                
                # Format price and performance values
            ...
            # Execute trade if strategy or action has changed and it's not HOLD
            if (strategy_name != last_strategy or action != last_action) and action != "HOLD":
                logger.info(f"Strategy changed from {last_strategy} to {strategy_name}")
                logger.info(f"Current BTC balance: {current_btc_balance}")
                logger.info(f"Action changed from {last_action} to {action}")
                
                # Update last known state
                last_strategy = strategy_name
                last_action = action
                
                # Get trade settings from app config
                ...
                # Calculate trade volume based on settings
                ...
                # Execute the trade based on the new strategy
                try:
                    # Check if we need to buy BTC before selling
                    if action.lower() == 'sell' and current_btc_balance < trade_volume:
                        ...
                        time.sleep(2)  # Wait for the order to settle
                        balance_data = get_account_balance()  # Update balances
                        current_btc_balance = float(balance_data.get('XXBT', '0.0'))
                    
                    # Adjust sell volume to available BTC
                    ...
                    # Update market data queue with trade result
                ...
                # Put market analysis in queue for UI updates
                market_data_queue.put({
                ...
                logger.info(f"Market monitor update - Mode: {trading_mode}, Price: {current_price}, "
                           f"Change: {price_change:.2f}%, Action: {action}, Patterns: {patterns}")
            
            # Update balances after trade execution
            balance_data = get_account_balance()
            if balance_data.get('error'):
                logger.error(f"Error updating balances: {balance_data['error']}")
            else:
...
def initialize():
    """Initialize Kraken API clients with XTBUSD symbol."""
    # Define parameters
    symbol = "XTBUSD"  # Kraken's Bitcoin/USD pair
    interval = 60  # 1-hour candles
    since = 1622505600  # Example UNIX timestamp

    # Fetch historical data from Kraken
    historical_data = KrakenAPI.fetch_historical_data(symbol, interval, since)
